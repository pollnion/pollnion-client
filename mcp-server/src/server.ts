...
server.tool(
  'analyze_file',
  'Analyze a TypeScript/TSX file and extract imports, exports, and structure',
  z.object({
  ...
  async (args) => {
    ...
    const exports =
      content.match(/export\s+(default\s+)?(function|const|class|interface|type)\s+(\w+)/g) || [];
...
server.tool(
  'find_files',
  'Find files matching a pattern (components, hooks, utils, tests, etc.)',
  z.object({
    type: z
...
server.tool(
  'analyze_component',
  'Detailed analysis of a React component',
  z.object({
...
server.tool(
  ...
  async (args) => {
    ...
    const state = (args.state as any) || 'open';
    ...
    const formatted = issues.data.map((issue: any) => ({
      number: issue.number,
      title: issue.title,
      state: issue.state,
...
server.tool(
  ...
  async (args) => {
    ...
    const formatted = prs.data.map((pr: any) => ({
      number: pr.number,
      title: pr.title,
      state: pr.state,
          ...
          text: JSON.stringify(
            {
              ...
              labels: pr.data.labels.map((l: any) => (typeof l === 'string' ? l : l.name)),
            },
            null,
            2
          ),
...
server.tool(
  ...
  async (args) => {
    ...
    const formatted = commits.data.map((commit: any) => ({
